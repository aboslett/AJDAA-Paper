...
# Goal: Overarching goal is to set up the drug overdoses database on an annual basis
...
for(fff in 1999:2002) {
  ...
  temp <- read.csv(file = paste0(remote_drive_csv, '/', 'mort', as.character(fff), '.csv'))
  ...
  temp %<>% mutate(
    ...
    white = ifelse(race == 1, 1, 0), # consistent across years
    black = ifelse(race == 2, 1, 0), # consistent across years
    indian = ifelse(race == 3, 1, 0), # consistent across years
    asian = ifelse(race > 3, 1, 0), # consistent across years
    ...
    dead_hospital_in = ifelse(placdth == 1, 1, 0), # consistent across years
    dead_hospital_out = ifelse(placdth == 2, 1, 0), # consistent across years
    dead_hospital_doa = ifelse(placdth == 3, 1, 0), # consistent across years
    # dead_hospital_unk = ifelse(placdth == 4, 1, 0), # unknown patient status; category doesn't exist 2003-on; drop?
    dead_nursing_home = ifelse(placdth == 5, 1, 0), # consistent across years
    ...
    age_20 = ifelse(ager52 < 30, 1, 0), # consistent across years
    age_30 = ifelse(ager52 == 30 | ager52 == 31, 1, 0), # consistent across years
    age_40 = ifelse(ager52 == 32 | ager52 == 33, 1, 0), # consistent across years
    age_50 = ifelse(ager52 == 34 | ager52 == 35, 1, 0), # consistent across years
    age_60 = ifelse(ager52 == 36 | ager52 == 37, 1, 0), # consistent across years
    age_70 = ifelse(ager52 == 38 | ager52 == 39, 1, 0), # consistent across years
    age_80 = ifelse(ager52 == 40 | ager52 == 41, 1, 0), # consistent across years
    age_125 = ifelse(ager52 > 41 & ager52 <= 51, 1, 0), # consistent across years
    age_unknown = ifelse(ager52 == 52, 1, 0), # consistent across years
    ...
    weekday = ifelse(weekday == 9, NA, weekday) # consistent across years
  )
  ...
  # Add counties FIPS code as string variables --------------------------------
  ...
  # Summarise drug-related information ----------------------- 
  # Note: Add indicators of drug usage in drug overdoses.
  
  temp %>% select(id_var, starts_with('record')) %>% 
    melt(id.vars = 'id_var') %>% 
    select(-variable) %>% 
    mutate(
      narcotic = str_detect(value, pattern = 'T40'),
      any_opioid = str_detect(value, pattern = 'T400|T401|T402|T403|T404|T406'),
      opium = str_detect(value, pattern = 'T400'),
      heroin = str_detect(value, pattern = 'T401'),
      natural_opioid = str_detect(value, pattern = 'T402'),
      methadone = str_detect(value, pattern = 'T403'),
      synthetic_opioid = str_detect(value, pattern = 'T404'),
      other_opioid = str_detect(value, pattern = 'T402|T404'),
      cocaine = str_detect(value, pattern = 'T405'),
      other_narcotic = str_detect(value, pattern = 'T406'),
      marijuana = str_detect(value, pattern = 'T407'),
      lsd = str_detect(value, pattern = 'T408'),
      hallucinogen = str_detect(value, pattern = 'T409'),
      sedative = str_detect(value, pattern = 'T42'),
      benzodiazpine = str_detect(value, pattern = 'T424'),
      other_sedative = str_detect(value, pattern = 'T420|T421|T422|T423|T425|T426|T427|T428'),
      psychotropic = str_detect(value, pattern = 'T43'),
      antidepressant = str_detect(value, pattern = 'T430|T431|T432'),
      antipsychotic = str_detect(value, pattern = 'T433|T434|T435'),
      psychostim = str_detect(value, pattern = 'T436'),
      antidepressant = str_detect(value, pattern = 'T430|T431|T432'),
      unidentified_drug = str_detect(value, pattern = 'T509'),
      other_drug = str_detect(value, pattern = 'T36|T37|T38|T39|T41|T438|T439|T44|T45|T46|T47|T48|T49|T500|T501|T502|T503|T504|T505|T506|T507|T508'),
      alcohol = str_detect(value, pattern = 'T51'),
      ethanol = str_detect(value, pattern = 'T510')
    ) %>% 
    group_by(id_var) %>%
    summarise_at(vars(narcotic:ethanol),
                 funs(max(.))) %>%
    ungroup() -> temp_drug
  
  temp %<>% left_join(temp_drug, by = c('id_var'))
  
  # Add indicator of drug death or not
  
  temp %<>% mutate(
    drug_poisoning_death = grepl(ucod, pattern = 'X40|X41|X42|X43|X44|X60|X61|X62|X63|X64|X85|Y10|Y11|Y12|Y13|Y14', ignore.case = TRUE)
  )
  
  temp %<>% select(-sex, -marstat, -state_fips_occ, -state_fips_res, -injwork)
  
  # Save deaths due to drug overdoses as their own RDS -------------------
  # Note: Filter by underlying cause of death (from NCHS/CDC defintions).
  
  temp %>% filter(ucod %in% c('X40', 'X41', 'X42', 'X43', 'X44', 'X60', 'X61', 'X62', 
                              'X63', 'X64', 'X85', 'Y10', 'Y11', 'Y12', 'Y13', 'Y14')) -> temp_drug_deaths
  
  temp_drug_deaths %>% saveRDS(paste0(remote_drive_rds, '/mort_drug_', as.character(fff), '.rds'))
  
  # Save as RDS file ----------------
  # Note: Save all other mortality as a yearly file
  
  temp %>% saveRDS(file = paste0(remote_drive_rds, '/', 'mort', as.character(fff), '.rds'))
...
for(fff in c(2003:2019)) {
  ...
  temp %<>% mutate(
    ...
    dead_hospital_out = ifelse(placdth == 2, 1, 0), # consistent across years
    dead_hospital_doa = ifelse(placdth == 3, 1, 0), # consistent across years
    dead_hospice = ifelse(placdth == 5, 1, 0), # consistent across years
    dead_nursing_home = ifelse(placdth == 6, 1, 0), # consistent across years
  ...
  # Note: Should have used maps package for this one. This will take some time for those following along.
  # One can change this code to import state FIPS code data frame. 
  ...
  # Note: As strings for merging sake (leading zeros - common issue with FIPS code).
  ...
  # Summarise drug-related information -----------------
  # Note: Indicators for drug involvement in deaths.
  
  temp %>% select(id_var, starts_with('record_')) %>% 
    melt(id.vars = 'id_var') %>% 
    select(-variable) %>% 
    mutate(
      narcotic = str_detect(value, pattern = 'T40'),
  ...
  temp %<>% mutate(
    drug_poisoning_death = str_detect(ucod, pattern = 'X40|X41|X42|X43|X44|X60|X61|X62|X63|X64|X85|Y10|Y11|Y12|Y13|Y14')
  )
...
# (3) Merge all yearly files into one data frame ------------------

rm(list = ls())

drug_overdoses <- data.frame()

for(fff in 1999:2016) {  
  
  temp <- readRDS(paste0(remote_drive_rds, '/', 'mort_drug_', as.character(fff), '.rds'))  
  
  temp %<>% mutate_at(
    vars(narcotic:drug_poisoning_death),
    funs(as.numeric(.))
  )
  
  drug_overdoses %<>% bind_rows(temp)
  
  rm(temp)
  
}

# Keep only drug poisoning deaths

names(drug_overdoses)

drug_overdoses$max <- do.call(pmax, drug_overdoses[70:91])

drug_overdoses %<>% filter(drug_poisoning_death == 1 | max > 0)

drug_overdoses %<>% select(-max)

# Add a new key variable based on id_var and year (n_distinct() == nrow())

drug_overdoses %<>% mutate(id_year = paste0(as.character(id_var), "-", as.character(year)))

# Add a variable indicating number of conditions

drug_overdoses %>% select(id_year, starts_with('record_')) %>%
  melt(id.vars = 'id_year') %>% 
  select(-variable) %>%
  filter(!is.na(value)) %>%
  unique() %>%
  group_by(id_year) %>%
  summarise(
    num_of_records = n() # Leads to actual number of records + 1 (I think it's fixed later, but it's inconsequential)
  ) %>%
  ungroup() -> number_of_records

drug_overdoses %<>% left_join(number_of_records, by = c('id_year'))

rm(number_of_records)

# Add variables to indicate unidentified drug overdoses (i.e., T509) -------------------

names(drug_overdoses)

drug_overdoses %<>% mutate(
...
# Add illicit-drug overdose, following Chris Ruhm
# Note: Not used elsewhere, but we followed along with Chris Ruhm's code in spirit. That code was in written
# in the Stata language.

drug_overdoses %<>% mutate(illicit = ifelse(heroin == 1 | synthetic_opioid == 1, 1, 0))

# Save file as an intermediate file ---------------------------------

drug_overdoses %>% saveRDS(remote_drive_rds, '/', 'mort_drug_intermediate_1.rds')